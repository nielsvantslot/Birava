...
**The RSC branch only notifies when the revalidated content actually changed — this is load-bearing, not an optimization.** `router.refresh()` sends the exact same `RSC: 1`-headers fetch a real `<Link>` transition would; the SW has no way to tell them apart. Notifying unconditionally on every successful revalidation created a closed loop with no exit condition: `router.refresh()`'s own fetch always found a cache hit, which always notified on completion, which always made the listener call `router.refresh()` again — as fast as the network round-trip allowed, easily 10+ cycles/second. Each cycle runs Next's router-sync effect once, which calls `history.replaceState()`; enough cycles inside 10 seconds tripped Safari's built-in rate limit on that API (100 calls/10s) and crashed the page with a `SecurityError` (caught via `ClientErrorLog` — see "Client-side error capture" below — after it went unnoticed on Chrome/Android, which has no equivalent hard throttle and just ran the same loop silently). The fix compares the freshly-fetched body against what's already cached and only calls `notifyClientsOfRevalidation` when they differ, so a refresh whose result turns out identical — which is what every cycle after the first genuine correction was — has nothing left to correct and the chain stops on its own.
