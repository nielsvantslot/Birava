...
**A new SW taking over mid-session used to leave the open tab running stale JS with no way back.** `skipWaiting()` + `clients.claim()` hand an already-open tab to the new SW immediately, with no navigation involved — the tab's own hydrated bundle doesn't change just because its SW did, so it kept running old code against a new cache generation until the user happened to navigate. `ServiceWorkerRegistration` now listens for `controllerchange` and does a one-time hard `location.reload()` through `lib/swControllerReload.ts`'s `ControllerChangeReloadGate`, which distinguishes a fresh install's first `controllerchange` (no controller yet — nothing stale, skip it) from a genuine mid-session takeover (already had a controller — reload), and never fires more than once per tab regardless of how many further events arrive. Deliberately a hard reload, not the reactive `postMessage`-driven `router.refresh()` pattern removed above — that pattern's exact feedback-loop failure mode doesn't apply here since a hard reload doesn't feed back into anything that could retrigger it.
...
**What this does not solve:** a genuinely first-time visitor — nothing installed, nothing cached, no account yet — still needs at least one successful round trip to sign up; there's no offline-capable identity system. The offline story here is "resume where you left off with no signal," not "onboard a brand-new user with zero connectivity ever." The **check-in itself** is the one action that's fully offline-safe regardless: `lib/offline/pendingCheckins.ts` queues it in IndexedDB the instant "Log drink" is tapped, synced later by `PendingCheckinsSync`. Foreground-only — WebKit has no Background Sync API — triggered on mount, on `online`, on the tab regaining visibility, and on a 45s interval as a backstop for a connection that never actually flips `navigator.onLine` (a captive portal, a flaky/degraded network, a transient server error) while the tab stays open and foregrounded the whole time.

**Every `PendingCheckin` carries the `userId` it was queued under, and every read is scoped to it.** This store is one shared IndexedDB database per device/browser, not per account — without the scoping, a shared or handed-off device (a different person logging in after a previous user logged out, or logging out while something is still queued) could flush the previous user's still-queued check-in, real photo/venue/drink and all, under whoever happens to be logged in the next time a sync trigger fires. An entry with no `userId` at all (queued by a build that predates this field) is treated as belonging to whoever asks, matching the pre-fix behavior, so this only ever matters for a brief window right after this shipped.
...
**Overlapping calls to `flushPendingCheckins` coalesce into a rerun instead of being dropped.** A module-level `flushing` guard already prevented two passes from running concurrently, but a plain `if (flushing) return` silently discarded a call that arrived while a pass was mid-flight — that pass's queue snapshot was taken *before* the new call, so it can't see whatever just changed (a check-in queued while a photo upload is still in progress, a manual "Retry now" flipping a failed entry back to "queued"). On an otherwise-healthy connection, the dropped work just sat as "Queued" until the next external trigger (`online`, tab-visibility, `PendingCheckinsSync`'s poll) — a real, previously-reported "check-in stuck in queue despite having a connection" symptom. `rerunRequested` now makes the in-flight call run one more full pass immediately after it finishes, so overlapping calls still never run concurrently but never just lose their work either.

**Each queued entry's own `id` doubles as an idempotency key, threaded to `addDrink` as `clientId`.** A retried sync — a client timeout where the server actually committed, or two tabs flushing the same queued entry at once — reaches `createDrinkEntry` (`lib/commands/drinkEntryCommands.ts`) a second time with the same id. Inside the same per-user advisory-locked transaction that already serializes session assignment, a lookup for an existing `{ id, userId }` row short-circuits into replaying the original success result instead of creating a second `DrinkEntry` and re-running session assignment against its own already-created neighbour. Scoped to `{ id, userId }`, not `id` alone: `clientId` is attacker-reachable input (`addDrink` is a real POST-backed Server Action, not just the offline queue's own caller), and `DrinkEntry` ids are visible to other users via shareable `/sessions/[id]` links, so an id-only lookup would let a caller "replay" — and silently discard their own submitted data in favor of — an entry they don't own.
...
**The pending-checkins panel (`components/drink/pending-checkins-panel.tsx`) shows aggregate batch progress, not just each entry's own status (added 2026-08-13).** With several check-ins queued, watching them flip to "Syncing…" one at a time with no sense of overall progress reads as "stuck," even when the sync itself is healthy. `useBatchProgress` tracks this purely from the reactive `entries` snapshot the panel already re-renders on (`onPendingCheckinsChanged`) — no new event from `flushPendingCheckins` needed: `batchTotal` captures the active (queued + syncing, excluding "failed") count the moment it goes from 0 to >0, keeps growing if more get queued mid-flush (so a check-in logged while a sync is already running extends the total instead of under-reporting it), and resets to `null` once nothing's active. "Syncing N of Total…" (`completedInBatch + 1` of `batchTotal`) only renders once there's more than one item in the batch and something is actively mid-flight — a lone item already says "Syncing…" inline, and the aggregate line would just repeat that. `completedInBatch` is derived by diffing which *ids* have disappeared from `entries` entirely since they were last seen active, not from the gap between `batchTotal` and the current active count — a failed entry stays present in `entries` (status `"failed"`), so a plain count-gap would count a failure as a completion (it shrinks the active count the same way a success does); id-diffing doesn't.
...
## First-run onboarding (added 2026-08-13)

Signup (`app/(auth)/signup/page.tsx`) auto-logs the new account in, then redirects to `app/onboarding` instead of straight to `/dashboard`. A short, skippable six-step explainer (`app/onboarding/onboarding-flow.tsx`, a client component) covers the things a first-time user has no way to already know: what Birava actually is (the elevator pitch — "Strava for a night out"), that sessions group themselves (no manual start/stop), that the dashboard is a shared feed of people you follow (not just your own log — illustrated with a small static mockup, `FeedPreview`, since this is the one concept worth showing rather than just describing), that crews are the social/leaderboard layer, that achievements/stats track variety never volume (including the specific one-rest-week-survives-two-doesn't streak grace rule — CLAUDE.md's locked `activeWeeks()` behavior), and — the last step — that push notifications exist and are worth turning on. `app/onboarding/page.tsx` re-checks auth itself (`getCurrentUser()`, `redirect("/login")` if absent) rather than relying solely on the middleware gate, matching the defense-in-depth pattern every `*Loader` in `app/(app)/layout.tsx` already uses. `app/onboarding/layout.tsx` duplicates `(auth)`'s minimal centered-card shell rather than nesting inside that route group — deliberately, so `/onboarding` (a logged-in page) can never be swept up by `proxy-session.ts`'s "already logged in, on an auth page → redirect to /dashboard" rule, which is presumed to key off exact pathnames but wasn't worth the risk of sharing a folder with.

**The notifications step reuses `PushSubscribeToggle` as-is** (`components/notifications/push-subscribe-toggle.tsx` — its own subscribe/timeout/permission-state handling, unchanged) rather than duplicating any of that logic. That component was otherwise only reachable buried at the bottom of `/profile`, so surfacing it here doubles as a fix for that discoverability gap. Toggling it is optional — "Log your first drink" still finishes the flow whether or not the user enabled it.

**Deliberately not middleware-gated.** `User.hasCompletedOnboarding` (migration `20260813152142_add_user_has_completed_onboarding`, plain additive `Boolean @default(false)`) is set by `completeOnboarding` (`lib/commands/userCommands.ts`, wrapped by `lib/controllers/onboardingController.ts`) on **both** finishing the last step and clicking "Skip" — either way there's nothing left to show. Nothing currently reads it to force a redirect: the only path that ever reaches `/onboarding` is signup's own one-time push. Threading the flag into `SessionUserDTO`/`getCurrentUser()`/`proxy-session.ts` (both of which independently `select` a fixed column list — see "Layered request flow") to support a hard "can't proceed until onboarded" wall would touch several auth-critical files for a feature that doesn't need it; skipped as scope the task didn't call for. The column exists as a real, honest record of what happened regardless — not derived, so it survives independent of the redirect logic ever changing — and is a natural hook if a forced gate or a "replay the intro" settings entry is ever wanted later.

Skipping lands on `/dashboard` (whose existing empty state already nudges toward logging a first drink); finishing the last step's own CTA goes straight to `/log` — clicking through to the end signals more intent than skipping does, so it gets the more decisive send-off.
