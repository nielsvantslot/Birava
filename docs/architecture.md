...
Signup (`app/(auth)/signup/page.tsx`) auto-logs the new account in, then redirects to `app/onboarding` instead of straight to `/dashboard`. A short, skippable three-step explainer (`app/onboarding/onboarding-flow.tsx`, a client component) covers the three ideas a first-time user has no way to already know: sessions group themselves (no manual start/stop), crews are the social/leaderboard layer, and achievements track variety, never volume — the last of Birava's product invariants a marketing-style intro screen could otherwise get wrong by accident. `app/onboarding/page.tsx` re-checks auth itself (`getCurrentUser()`, `redirect("/login")` if absent) rather than relying solely on the middleware gate, matching the defense-in-depth pattern every `*Loader` in `app/(app)/layout.tsx` already uses. `app/onboarding/layout.tsx` duplicates `(auth)`'s minimal centered-card shell rather than nesting inside that route group — deliberately, so `/onboarding` (a logged-in page) can never be swept up by `proxy-session.ts`'s "already logged in, on an auth page → redirect to /dashboard" rule, which is presumed to key off exact pathnames but wasn't worth the risk of sharing a folder with.
