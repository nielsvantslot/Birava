...
**Session reminders got a real per-user algorithm (added 2026-08-08)**, replacing a single global "quiet for 1h, remind once" rule. `sendSessionReminders()` (`lib/commands/sessionReminderCommands.ts`) still finds candidate sessions with `endedAt` inside the `SESSION_GAP_MS` window, but everything past that is now personalized (pure math lives in `lib/sessionReminderAlgorithm.ts`, data helpers in `lib/queries/reminderAlgorithmQueries.ts`):

- **Personalized timing** — a user's own median intra-session check-in gap (from `getDrinkHistory` + `groupIntoSessions`), clamped to `[30min, 90min]`, replaces the fixed 1h threshold. A user with no history yet falls back to that same 60min default.
- **Escalation** — up to 3 reminders per session at increasing tiers off that personalized threshold (tier 2 ≈2x, tier 3 ≈3.5x, both capped to leave a buffer before `SESSION_GAP_MS` permanently closes the session), instead of a single one-shot nudge. Dedup is now a per-session *count* of existing `SESSION_REMINDER` rows rather than a boolean exists-check, and each tick only ever sends the next tier due — a long gap in ticks catches up one reminder, not a burst of several at once.
- **Engagement cap** — how many tiers a user is eligible for is capped by how often they've actually opened their last 10 reminders (`Notification.openedAt`, set by `markNotificationOpened` when a user clicks through a specific notification — added alongside the algorithm as the first true per-notification engagement signal, distinct from the bulk `readAt` `markAllRead` sets for the whole list). A user with no engagement history gets the benefit of the doubt (2); a consistently-unresponsive user (0 opens over ≥3 resolved reminders) stays capped at exactly the old single-reminder behavior — nobody gets nudged *more* than before, only a responsive user gets the extra tiers.
