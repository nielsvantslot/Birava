...
- **Testing a production build locally** (e.g. to get the service worker to actually register — it's a no-op in dev, see above): `docker-compose.yml` hardcodes `NODE_ENV: development` on the `app` service for the dev command. That value leaks into `docker compose run app sh -c "npm run build"` unless overridden, and a non-`production` `NODE_ENV` during `next build` trips a confirmed upstream Next.js bug (Next 15.5.20 reproduced; also hits fresh `create-next-app` projects — see vercel/next.js#77261) that crashes prerendering the auto-generated `/404`/`/500` pages with `Error: <Html> should not be imported outside of pages/_document`, unrelated to any app code. Always run `docker compose run --rm -e NODE_ENV=production app sh -c "npm run build"` (and `... start`) for a local production build/test — don't chase this error in app code, it isn't there.
